...
setwd("E:/Fall Term/SYS 6018 Applied Data Mining/Project")
...
setwd("E:/Fall Term/SYS 6018 Applied Data Mining/Project/Data/FARS2016NationalCSV")
files16 = list.files(pattern=".*csv")
acc16 = read_csv(files16[1])
veh16 = read_csv(files16[20])
per16 = read_csv(files16[17])
setwd("E:/Fall Term/SYS 6018 Applied Data Mining/Project/Data/FARS2017NationalCSV")
...
cor(accident$A_PED, accident$A_PED_F)
#[1] 0.9941502
cor(accident$A_PEDAL, accident$A_PEDAL_F)
#[1] 0.9952215
accident$A_PED = NULL
...
# CLEANING AND SUMMARISING VEHICLE DATASET
...
# CLEANING AND SUMMARISING PERSON DATASET
...
# Removing vehicle number and person number from person
person$VEH_NO = NULL
person$PER_NO = NULL
...
ggplot(tsne_res, aes(x = V1, y = V2, color = severity)) + geom_point() + ggtitle("First and Second t-SNE dimensions colored by severity") + theme(plot.title = element_text(hjust = 0.5))
